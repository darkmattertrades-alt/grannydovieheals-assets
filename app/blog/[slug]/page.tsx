...
export async function generateStaticParams() {
  ...
  return [
    ...slugs.map((slug) => ({ slug })),
    ...categoryKeys.map((slug) => ({ slug })),
  ]
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  // Check if it is a category page
  const category = CATEGORIES[params.slug as BlogCategory]
  if (category) {
    return {
  ...
  // Otherwise treat as a blog post
...
export default function BlogSlugPage({ params }: Props) {
  ...
  const category = CATEGORIES[params.slug as BlogCategory]
  if (category) {
    const posts = getPostsByCategory(params.slug as BlogCategory)
    return (
